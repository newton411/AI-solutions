 ...
 * - Spartan warrior mascot integration
 * - Refined cyan/blue color scheme with preserved layout
...
export const HeroSection: React.FC<HeroSectionProps> = ({ onGetStarted }) => {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
      {/* Animated Background Orbs - Refined with Cyan instead of Gold */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {/* Spartan Hero Illustration */}
        <motion.div
          className="hidden lg:block absolute top-1/4 right-0 opacity-20 pointer-events-none"
          ...
          {[
            ...
            return (
              <motion.div
                key={feature.title}
                variants={itemVariants}
                className="group glass-cyan rounded-xl p-6 sm:p-8 hover:shadow-lg hover:shadow-cyan-500/20 relative overflow-hidden"
              >
                {/* Background gradient */}
        ...
        <motion.div
          className="mt-12 sm:mt-16 text-xs text-gray-600 text-center"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 2, duration: 0.5 }}
        >
          Secured by SUPLOCK • MEV-Protected • Audited Protocol
        </motion.div>
      </div>
    </div>
  );
